...
/**
*/
VkEffectSampler::VkEffectSampler()
{
	// empty
}
...
VkEffectSampler::~VkEffectSampler()
{
	vkDestroySampler(VkContext::currentContext, this->sampler, NULL);
}
...
*/
void
VkEffectSampler::Setup(const eastl::vector<InternalEffectVariable*>& textures)
{
	...
	VkResult res = vkCreateSampler(VkContext::currentContext, &info, NULL, &this->sampler);
	assert(res == VK_SUCCESS);

	unsigned i;
	for (i = 0; i < textures.size(); i++)
	{
		VkEffectVariable* var = static_cast<VkEffectVariable*>(textures[i]);
		var->immutableSampler = this->sampler;
	}
}
