...
GLSL4EffectVariable::Setup(eastl::vector<InternalEffectProgram*> programs, const eastl::string& defaultValue)
{
	...
	if (!this->isInVarblock)
	{
		...
		for (i = 0; i < programs.size(); i++)
		{
			...
			if (this->type >= Sampler1D && this->type <= SamplerCubeArray) opengl4Program->textureBindsCount = a_max(this->binding + 1, opengl4Program->textureBindsCount);
			if (this->type >= Image1D && this->type <= ImageCubeArray) opengl4Program->imageBindsCount = a_max(this->binding + 1, opengl4Program->imageBindsCount);
#endif
		...
		case SamplerCubeArray:
			{
				...
				{
#if GL4_MULTIBIND
					if (this->activeProgram->textureBinds[this->binding] != obj->bound.handle)
					{
						this->activeProgram->textureBinds[this->binding] = obj->bound.handle;
						this->activeProgram->texturesDirty = true;
					}
				}
				else
				{
					this->activeProgram->textureBinds[this->binding] = 0;
					this->activeProgram->texturesDirty = true;
				}
#else
                    if (this->bindless)
		...
		case Image2DMS:
            {
				...
				if (obj)
				{
					if (this->activeProgram->imageBinds[this->binding] != obj->bound.handle)
					{
						this->activeProgram->imageBinds[this->binding] = obj->bound.handle;
						this->activeProgram->imagesDirty = true;
					}
				}
				else
				{
					this->activeProgram->imageBinds[this->binding] = 0;
					this->activeProgram->imagesDirty = true;
				}
#else
                if (obj && obj->bound.textureType == this->textureType)
                {
					...
					glBindImageTexture(this->binding, obj->bound.handle, 0, GL_FALSE, 0, this->glAccessMode, this->glImageFormat);
                }
                else
                {
					glBindImageTexture(this->binding, 0, 0, GL_FALSE, 0, this->glAccessMode, this->glImageFormat);
                }
#endif
		...
		case ImageCubeArray:
			{
                ...
                if (obj && obj->bound.textureType == this->textureType)
                {
					...
					glBindImageTexture(this->binding, obj->bound.handle, 0, GL_TRUE, 0, this->glAccessMode, this->glImageFormat);
                }
                else
                {
					glBindImageTexture(this->binding, 0, 0, GL_TRUE, 0, this->glAccessMode, this->glImageFormat);
                }
#endif
