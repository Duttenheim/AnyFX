...
EffectProgramStreamLoader::Load(BinReader* reader, Effect* effect)
{
	...
	internalProgram->shaderBlock.vsBinarySize = reader->ReadUInt();
	internalProgram->shaderBlock.vsBinary = reader->ReadBytes(internalProgram->shaderBlock.vsBinarySize);
	...
	internalProgram->shaderBlock.hsBinarySize = reader->ReadUInt();
	internalProgram->shaderBlock.hsBinary = reader->ReadBytes(internalProgram->shaderBlock.hsBinarySize);
	...
	internalProgram->shaderBlock.dsBinarySize = reader->ReadUInt();
	internalProgram->shaderBlock.dsBinary = reader->ReadBytes(internalProgram->shaderBlock.dsBinarySize);
	...
	internalProgram->shaderBlock.gsBinarySize = reader->ReadUInt();
	internalProgram->shaderBlock.gsBinary = reader->ReadBytes(internalProgram->shaderBlock.gsBinarySize);
	...
	internalProgram->shaderBlock.psBinarySize = reader->ReadUInt();
	internalProgram->shaderBlock.psBinary = reader->ReadBytes(internalProgram->shaderBlock.psBinarySize);
	...
	internalProgram->shaderBlock.csBinarySize = reader->ReadUInt();
	internalProgram->shaderBlock.csBinary = reader->ReadBytes(internalProgram->shaderBlock.csBinarySize);
	...
	for (i = 0; i < numOffsets; i++)
	{
		...
		internalProgram->variableBlockOffsets[var] = offset;
	}
